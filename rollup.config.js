...
// NOTE: This value must be defined outside of the plugin because it needs to persist from build to build (e.g. the
// module and nomodule builds). If, in the future, the build process were to extends beyond just this rollup config,
// the manifest would have to be initialized from a file. However, since everything  is currently being built here, 
// it's okay to just initialize it as an empty object when the build starts.
const manifest = {};

/**
 * A Rollup plugin to generate a manifest of chunk names to their filenames (including their content hash). This
 * manifest is then used by the template to point to the current URL.
 * @return {Object}
 */
...
/**
 * A Rollup plugin to generate a list of import dependencies for each entry point in the module graph. This is then
 * used by the template to generate the necessary `<link rel="modulepreload">` tags.
 * @return {Object}
 */
...
const moduleConfig = {
  input: {
    'terra.development': 'src/index.mjs',
  },
  output: {
